async function CreateProject(projectName) {
    const response = await fetch(`/project/${projectId}/CrProject`)
}

let btnFind = document.getElementById("btnFind")
let idInpt = document.getElementById("idInpt")
btnFind.addEventListener('click', ()=>{
    loadProject(idInpt.value)
})

let btnCrPr = document.getElementById("btnCrPr")
let nameInp = document.getElementById("namePr")
btnCrPr.addEventListener('click', async ()=>{
    await fetch(`/createPr/${nameInp.value}`)
})
